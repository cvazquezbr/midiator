...
async function fetchStatsWithRefresh(fetch, userId, initialAccessToken, postsByAuthor) {
    ...
    const processAuthor = async (authorUrn, posts, token) => {
        ...
        // For organizations, we can batch 'share' URNs.
        if (authorUrn.includes(':organization:') && shareUrns.length > 0) {
            ...
            results.push(res);
        } else if (shareUrns.length > 0) {
            // For personal authors, or orgs with only non-batchable posts, process shares individually.
            ugcPostUrns.push(...shareUrns);
        }

        // Process all ugcPosts and any individual shares individually.
        if (ugcPostUrns.length > 0) {
            console.log(`[Analytics] Processing ${ugcPostUrns.length} URNs individually for author ${authorUrn}`);
