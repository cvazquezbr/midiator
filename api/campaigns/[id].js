  ...
  if (req.method === 'GET') {
    try {
      const { rows } = await query(
        'SELECT id, name, campaign_data, updated_at FROM campaigns WHERE id = $1 AND user_id = $2',
        [id, userId]
      );
      if (rows.length === 0) {
    ...
    try {
      const { name, campaign_data } = await parseBody(req);
      if (!name || !campaign_data) {
        return res.status(400).json({ error: 'Campaign name and data are required.' });
      }
      const { rows } = await query(
        'UPDATE campaigns SET name = $1, campaign_data = $2, updated_at = NOW() WHERE id = $3 AND user_id = $4 RETURNING id, name, updated_at',
        [name, campaign_data, id, userId]
      );
      if (rows.length === 0) {
        return res.status(404).json({ error: 'Campaign not found or access denied.' });
      }
