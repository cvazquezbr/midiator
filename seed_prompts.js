...
// To run this script, you need to have node-postgres installed (`npm install pg` or `yarn add pg`).
// You also need to have the POSTGRES_URL environment variable set, for example, in a .env file.
// Use `node -r dotenv/config seed_prompts.js` to run it with dotenv support.
...
const promptsToSeed = [
  {
    ...
    description: 'Generates the main content for a LinkedIn campaign post.',
    prompt_text: `
...
Gere uma resposta JSON com os seguintes campos: "titulo" (string), "conteudo" (string), "cta" (string), e "hashtags" (string, separadas por vírgula). A resposta deve ser apenas o JSON.`
  ...
  {
    name: 'generateCampaignImagePrompt',
    description: 'Generates a detailed text prompt for an image generation model (e.g., DALL-E) to create a campaign background image.',
    prompt_text: `
  ...
  },
  {
    ...
    description: 'Generates the final prompt sent to the image generation API, combining the image prompt with color and aspect ratio constraints.',
    prompt_text: `
    {prompt}
    {colorPalettePrompt}
  ...
  {
    name: 'generateFormattedContent',
    description: 'Formats post content into HTML for a WordPress blog.',
    prompt_text: `
  ...
  },
  {
    ...
    description: 'Generates a sequence plan for follow-up posts based on a main piece of content.',
    prompt_text: `Você é um estrategista de marketing de conteúdo. Sua tarefa é criar um plano para {neededQuantity} novos posts sequenciais no LinkedIn, baseados em um conteúdo principal e complementando os posts já existentes.
...
`
  ...
  {
    name: 'generateFollowupPosts',
    description: 'Generates the content for a single follow-up post based on a plan.',
    prompt_text: `
...
`
  ...
  {
    name: 'generateCommonSolutions',
    description: 'Generates a list of common solutions for a given problem and persona.',
    prompt_text: `
  ...
  {
    name: 'generateCommonProblems',
    description: 'Generates a list of common problems for a given persona and author.',
    prompt_text: `
  ...
  {
    name: 'generateIAContent',
    description: 'Generates content for an Instagram carousel from a base text.',
    prompt_text: `A partir do TEXTO BASE fornecido abaixo, gere conteúdo para um carrossel de Instagram com {promptNumRecords} elementos.
...
Lembre-se: Sua resposta final deve conter APENAS o bloco \`\`\`csv ... \`\`\` com os dados.`
  },
  {
    ...
    description: 'Generates a 5-color palette based on a user briefing and color psychology.',
    prompt_text: `Crie uma paleta harmoniosa de 5 cores baseada no briefing abaixo, aplicando princípios da psicologia das cores na cultura ocidental.
...
async function seed() {
  ...
  try {
    ...
    for (const prompt of promptsToSeed) {
      // Use ON CONFLICT to prevent errors if the script is run multiple times.
      // It will update the description and prompt_text if the name already exists.
      const queryText = `
        ...
        DO UPDATE SET
          ...
          updated_at = NOW();
      `;
      await client.query(queryText, [prompt.name, prompt.description, prompt.prompt_text]);
      console.log(`- Seeded/Updated prompt: ${prompt.name}`);
    }
