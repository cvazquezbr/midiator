 ...
 * @param {object} promptData - O objeto com os dados do prompt ({ persona, autor, instrucoes, formato, aspectRatio, followup_posts_quantity }).
 */
 ...
 * @returns {{persona: string, autor: string, instrucoes: string, formato: string, aspectRatio: string, followup_posts_quantity: number}} O objeto do prompt ou um objeto com campos vazios.
 */
export function getCampaignPrompt() {
  if (typeof window !== 'undefined' && window.localStorage) {
    try {
      const storedData = window.localStorage.getItem(CAMPAIGN_PROMPT_STORAGE_KEY);
      const defaultPrompt = { persona: '', autor: '', instrucoes: '', formato: '', aspectRatio: '1:1', followup_posts_quantity: 5 };
      ...
      try {
        ...
        if (typeof parsedData === 'object' && parsedData !== null) {
          // Garante que o valor de followup_posts_quantity seja um número e tenha um fallback
          const finalData = {
            ...defaultPrompt,
            ...parsedData,
          };
          if (typeof finalData.followup_posts_quantity !== 'number') {
            finalData.followup_posts_quantity = 5;
          }
          return finalData;
        }
      } catch (e) {
      ...
      return { persona: '', autor: '', instrucoes: '', formato: '', aspectRatio: '1:1', followup_posts_quantity: 5 };
    }
  }
  return { persona: '', autor: '', instrucoes: '', formato: '', aspectRatio: '1:1', followup_posts_quantity: 5 };
}
