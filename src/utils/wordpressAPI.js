import { getWordpressConfig } from './wordpressCredentials';
...
/**
 * Processa as hashtags: verifica se existem no WordPress, cria as que não existem
 * e retorna uma lista de IDs de tags.
 * @param {string[]} hashtags - Array de strings de hashtags.
 * @param {object} config - O objeto de configuração do WordPress.
 * @param {Headers} headers - Os cabeçalhos da requisição.
 * @returns {Promise<number[]>} Uma promessa que resolve para um array de IDs de tags.
 */
const _processHashtags = async (hashtags, config, headers) => {
  const tagIds = [];
 ...
 * @param {object} campaignContent - O conteúdo da campanha.
 * @param {string} conteudoFormatado - O conteúdo formatado em HTML.
 * @param {number[]} tagIds - Array de IDs de tags do WordPress.
 * @param {number} mediaId - O ID da imagem destacada.
 * @param {string} mediaUrl - A URL da imagem no WordPress.
 * @param {object} config - O objeto de configuração do WordPress.
 * @param {Headers} headers - Os cabeçalhos da requisição.
 * @returns {Promise<object>} Uma promessa que resolve para o objeto do post criado.
 */
const _createPost = async (campaignContent, conteudoFormatado, tagIds, mediaId, mediaUrl, config, headers) => {
  const wordpressUrl = config.wordpressUrl.replace(/\/$/, '');
...
export const publishToWordPress = async (campaignData) => {
  ...
  const tagIds = await _processHashtags(campaignContent.hashtags, config, headers);
  ...
  const postObject = await _createPost(campaignContent, conteudoFormatado, tagIds, mediaId, mediaUrl, config, headers);
