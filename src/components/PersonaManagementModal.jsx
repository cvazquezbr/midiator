...
const PersonaManagementModal = ({ open, onClose }) => {
  ...
  const drawerContent = (
    <Box sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'background.paper' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexShrink: 0 }}>
        ...
        <Button variant="contained" size="small" startIcon={<Add />} onClick={handleNewPersona} sx={{bgcolor: '#8b5cf6', '&:hover': { bgcolor: '#7c3aed' }}}>Nova</Button>
      </Box>
      <Divider />
  ...
  return (
    <Modal open={open} onClose={onClose} closeAfterTransition>
        <Paper sx={{ display: 'flex', height: '100vh', width: '100vw', bgcolor: 'transparent' }}>
            <Drawer
            ...
            <Box component="main" sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', height: '100vh' }}>
                <AppBar position="static" color="default" elevation={0} sx={{ borderBottom: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
                    <Toolbar>
                        ...
                        <Typography variant="h6" noWrap component="div" sx={{ flexGrow: 1 }}>
                            {selectedPersona?.name || 'Nova Persona'}
                        </Typography>
                ...
                <Box sx={{ flexGrow: 1, p: {xs: 2, md: 3}, overflow: 'auto', bgcolor: 'background.default' }}>
                    {selectedPersona ? (
