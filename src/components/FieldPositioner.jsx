...
const COMPLETE_DEFAULT_STYLE_FOR_FIELD_POSITIONER = { fontFamily: 'Arial', fontSize: 24, fontWeight: 'normal', fontStyle: 'normal', textDecoration: 'none', color: '#000000', textAlign: 'left', verticalAlign: 'top', lineHeightMultiplier: 1.2, textStroke: false, strokeColor: '#ffffff', strokeWidth: 2, textShadow: false, shadowColor: '#000000', shadowBlur: 4, shadowOffsetX: 2, shadowOffsetY: 2, backgroundColor: '#000000', backgroundOpacity: 0, };
import { isHtmlField } from '../lib/utils';
import { autoArrangeFields as autoArrangeFieldsUtil } from '../utils/autoArrange';
const FieldPositioner = ({ editorState, setEditorState, onImageDisplayedSizeChange, selectedField, setSelectedField, originalImageSize, darkMode, onOpenHtmlEditor, currentPreviewIndex, onFontScaleChange, isCropping, }) => { const { fieldPositions, fieldStyles, csvData, brandElements, pageTemplate } = editorState;
// Derive csvHeaders from csvData to make the component more robust
const csvHeaders = useMemo(() => { if (csvData && csvData.length > 0 && csvData[0]) { return Object.keys(csvData[0]); } return []; }, [csvData]);
...
const handleContentChange = useCallback((field, newText) => { setEditorState(prevState => { if (!prevState.csvData || prevState.csvData.length === 0) return prevState;
...
const handlePositionChange = useCallback((id, newPosition) => { setEditorState(prevState => { const { fieldPositions, pageTemplate, brandElements } = prevState;
  // Handle cropbox
  if (id === '__cropbox__') {
...
const handleSizeChange = useCallback((id, newSize) => { setEditorState(prevState => { const { fieldPositions, pageTemplate, brandElements } = prevState;
  // Handle cropbox
  if (id === '__cropbox__') {
    const newImages = pageTemplate.images.map(img =>
      img.id === selectedField
...
const handleContainerTouchStart = (e) => { if (!e.target.closest('.text-box')) { setIsInteracting(true); } };
const handleContainerTouchEnd = () => { setIsInteracting(false); };
useEffect(() => { if (renderedImageMetrics.width > 0 && effectiveImageSize?.width > 0) { const previewScale = renderedImageMetrics.width / effectiveImageSize.width; setFontScale(previewScale);
...
useEffect(() => { if (isInteracting) { document.body.style.overflow = 'hidden'; document.body.style.touchAction = 'none';
  return () => {
...
const completeFieldStyles = React.useMemo(() => { const safeHeaders = Array.isArray(csvHeaders) ? csvHeaders : []; const styles = {}; if (!fieldStyles) { safeHeaders.forEach(header => { styles[header] = { ...COMPLETE_DEFAULT_STYLE_FOR_FIELD_POSITIONER }; }); return styles; } safeHeaders.forEach(header => { styles[header] = { ...COMPLETE_DEFAULT_STYLE_FOR_FIELD_POSITIONER, ...(fieldStyles[header] || {}), }; }); return styles; }, [csvHeaders, fieldStyles]);
const renderableElements = React.useMemo(() => { const elements = [];
// Add page images
...
const getGradientCss = (gradient) => { if (!gradient) return 'none';
...
return ( <Box ref={containerRef} sx={{ width: '100%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden', }} > <Box className="text-container" sx={{ position: 'relative', background: backgroundValue, width: dynamicSize.width, height: dynamicSize.height, border: '2px solid #ddd', borderRadius: 2, overflow: 'hidden', display: 'block', margin: 'auto', }} onTouchStart={handleContainerTouchStart} onTouchEnd={handleContainerTouchEnd} > <Box className="elements-wrapper" sx={{ position: 'absolute', left: 0, top: 0, width: '100%', height: '100%', }} onClick={(e) => { if (e.target === e.currentTarget) { setSelectedField('page_background'); } }} onTouchStart={(e) => { if (e.target === e.currentTarget) { setSelectedField('page_background'); } }} > {renderableElements.map(element => ( <DraggableElement key={element.id} element={{ ...element.position, type: element.type, id: element.id }} position={element.position} style={element.style} content={element.content} isSelected={selectedField === element.id} onSelect={setSelectedField} onPositionChange={handlePositionChange} onSizeChange={handleSizeChange} containerSize={renderedImageMetrics} onContentChange={element.type === 'text' ? handleContentChange : undefined} onDoubleClick={() => { if (element.type === 'text' && isHtmlField(element.id)) { onOpenHtmlEditor(element.id); } }} rotation={element.rotation} originalImageSize={effectiveImageSize} fontScale={element.fontScale} enableHtmlRendering={element.enableHtmlRendering} darkMode={darkMode} pendingAssets={pendingAssets} /> ))} </Box> </Box> </Box> ); };
