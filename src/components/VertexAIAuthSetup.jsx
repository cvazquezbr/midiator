    ...
    <>
      <Box sx={{ p: 1 }}>
        ...
        <Typography variant="body2" gutterBottom sx={{mt: 2}}>
          Cole o conteúdo do seu arquivo JSON de Chave de Conta de Serviço do Google Cloud. Esta credencial é necessária para autenticação segura na API Vertex AI para geração de imagem.
          {' '}
        ...
        <FormControl fullWidth sx={{ mt: 2 }} disabled={loadingModels || errorModels}>
            <InputLabel id="gemini-image-model-select-label">Modelo de Imagem (Vertex AI)</InputLabel>
            <Select
            ...
            >
              <MenuItem value="models/gemini-pro-vision">gemini-pro-vision (Recomendado, via Gemini API)</MenuItem>
              {imageModels.map(model => (
                <MenuItem key={model.name} value={model.name}>{model.displayName}</MenuItem>
              ))}
               <MenuItem value="models/imagen-3.0-generate-002">imagen-3.0-generate-002 (Maior Custo)</MenuItem>
            </Select>
        </FormControl>
      ...
      <Dialog open={showInfobox} onClose={() => setShowInfobox(false)} fullWidth maxWidth="lg">
        <DialogTitle>
           <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            Instruções de Configuração da Vertex AI
...
export default VertexAIAuthSetup;
