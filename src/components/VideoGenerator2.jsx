...
const VideoGenerator2 = ({ generatedPages: generatedImages }) => {
  ...
  const generateVideoWithFFmpeg = async () => {
    const hasAudio = csvData && csvData.some(record => record.audio);

    const totalVideoFrames = generatedImages.reduce((acc, _, i) => {
      const record = csvData[i];
      const duration = record?.audio?.duration ? record.audio.duration : slideDuration;
    ...
    try {
      ...
      if (hasAudio) {
        for (const [i, record] of csvData.entries()) {
          const audioBlob = record.audio?.blob;
          if (audioBlob) {
      ...
      generatedImages.forEach((_, i) => {
        const record = csvData[i];
        let duration = (record?.audio?.duration ? record.audio.duration : slideDuration) + effectiveSlideDelay;
      ...
      generatedImages.slice(1).forEach((_, idx) => {
        ...
        const duration = (record?.audio?.duration ? record.audio.duration : slideDuration) + effectiveSlideDelay;
      ...
      const lastImageDuration = (lastRecord?.audio?.duration ? lastRecord.audio.duration : slideDuration) + effectiveSlideDelay + finalSlideDelay;
      ...
      if (hasAudio) {
        ...
        for (let i = 0; i < generatedImages.length; i++) {
          const record = csvData[i];
          const hasNarration = !!record.audio?.blob;
  ...
  const generateVideoPerRecord = async () => {
    ...
    try {
      ...
      for (let i = 0; i < generatedImages.length; i++) {
        ...
        const record = csvData[i];
        const framesForThisVideo = Math.floor((record?.audio?.duration || slideDuration) * fps);
