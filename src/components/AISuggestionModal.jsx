...
const AISuggestionModal = ({
  ...
  const renderSuggestionContent = (suggestion) => {
    ...
    // For default or string-based suggestions
    return (
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md" PaperProps={{ sx: { minHeight: '400px' } }}>
      ...
      <DialogContent dividers sx={{ display: 'flex', flexDirection: 'column' }}>
        {loading && (
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexGrow: 1, flexDirection: 'column', gap: 2, textAlign: 'center' }}>
        ...
        {!loading && !error && (
            ...
            <Grid container spacing={2} sx={{ pt: 2 }}>
                ...
                <Grid item xs={12} md={suggestionType === 'product' ? 12 : 6} key={index}>
                    <Paper
                        variant="outlined"
                        sx={{
                            p: 2,
