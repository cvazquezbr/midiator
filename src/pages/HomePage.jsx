        ...
        <Box component="main" sx={{ flexGrow: 1, p: { xs: 1, sm: 2, md: 3 }, transition: theme.transitions.create('margin', { easing: theme.transitions.easing.sharp, duration: theme.transitions.duration.leavingScreen }) }} >
          ...
          {currentView === 'campaigns' && (
            <>
              ...
              {activeStep === 5 && <AudioGenerator csvData={csvData} fieldPositions={fieldPositions} onAudiosGenerated={(audios) => setCampaignState(prev => ({ ...prev, generatedAudioData: audios }))} initialAudioData={campaignState.generatedAudioData} />}
              {activeStep === 6 && <VideoGenerator2 generatedPages={generatedPagesData} generatedAudioData={campaignState.generatedAudioData} generatedVideos={generatedVideos} onVideoGenerated={(assets) => { setCampaignState(p => ({ ...p, generatedVideos: [...p.generatedVideos, ...assets] })); addPendingAssetMap(Object.fromEntries(assets.flatMap(a => [[a.url, a.blob], [a.thumbnailUrl, a.thumbnailBlob]]).filter(e => e[0]))); }} onUpdateVideos={(videos) => setCampaignState(prev => ({ ...prev, generatedVideos: videos }))} onNewAsset={addPendingAsset} />}
              {activeStep === 7 && (
                <Publisher
                  settings={settings}
