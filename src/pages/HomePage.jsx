...
function HomePage() {
  ...
  const parseCsvFile = async (file) => { console.log('[HomePage] parseCsvFile - Arquivo recebido:', file); if (!file) return; try { const { data: newCsvData, headers: newHeaders } = await parseCsv(file); console.log('[HomePage] parseCsvFile - Dados retornados do parser:', { newCsvData, newHeaders }); if (newCsvData && newCsvData.length > 0) { console.log('[HomePage] parseCsvFile - CSV com dados, atualizando estado...'); setCsvData(newCsvData); setCsvHeaders(newHeaders); const updatedFieldPositions = {}; const updatedFieldStyles = {}; const defaultStylesBase = { fontFamily: 'Inter', fontSize: 24, fontWeight: 'normal', fontStyle: 'normal', textDecoration: 'none', color: '#000000', textStroke: false, strokeColor: '#ffffff', strokeWidth: 2, textShadow: false, shadowColor: '#000000', shadowBlur: 4, shadowOffsetX: 2, shadowOffsetY: 2, textAlign: 'left', verticalAlign: 'top' }; newHeaders.forEach((header, index) => { updatedFieldPositions[header] = fieldPositions[header] || { x: 10 + (index % 5) * 18, y: 10 + Math.floor(index / 5) * 12, width: 15, height: 10, visible: true }; if (fieldStyles[header]) { updatedFieldStyles[header] = fieldStyles[header]; } else { if (index === 0) { updatedFieldStyles[header] = { ...defaultStylesBase, fontFamily: 'Anton', fontSize: 72 }; } else { updatedFieldStyles[header] = { ...defaultStylesBase }; } } }); setFieldPositions(updatedFieldPositions); setFieldStyles(updatedFieldStyles); console.log('[HomePage] parseCsvFile - Estado atualizado.'); } else { console.log('[HomePage] parseCsvFile - CSV vazio ou sem dados. Nenhuma alteração de estado.'); } } catch (error) { toast.error(error.message || 'Ocorreu um erro desconhecido ao processar o arquivo CSV.'); console.error('[HomePage] parseCsvFile - Erro capturado:', error); } };
  ...
  const canProceedToStep = () => { switch (activeStep) { case 0: return campaignContent !== null; case 1: return csvData.length > 0; case 2: return backgroundImage !== null; case 3: return true; default: return true; } };
      ...
      <Box sx={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <Box component="main" sx={{ flexGrow: 1, p: { xs: 1, sm: 2, md: 3 }, transition: theme.transitions.create('margin', { easing: theme.transitions.easing.sharp, duration: theme.transitions.duration.leavingScreen }) }} >
          ...
          <div hidden={activeStep !== 1}>
            {console.log('[HomePage] Renderizando PostsCurtosStep com props:', { csvData, csvHeaders, inputMethod })}
          ...
          <div hidden={activeStep !== 3}><FieldPositioner backgroundImage={backgroundImage} csvHeaders={csvHeaders} fieldPositions={fieldPositions} setFieldPositions={setFieldPositions} fieldStyles={fieldStyles} setFieldStyles={setFieldStyles} csvData={csvData} onImageDisplayedSizeChange={setDisplayedImageSize} colorPalette={colorPalette} standardsColors={standardsColors} onCsvDataUpdate={handleCsvRecordContentUpdate} onSelectFieldExternal={setSelectedField} originalImageSize={originalImageSize} imageFilters={imageFilters} setImageFilters={setImageFilters} brandElements={brandElements} setBrandElements={setBrandElements} onZIndexChange={handleZIndexChange} /></div>
