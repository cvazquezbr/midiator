...
function HomePage() {
  ...
  const handleGenerateSummary = async (targetLength, content = campaignContent) => { if (!content?.conteudo) { toast.error("Por favor, gere o conteúdo principal primeiro."); return; } const setLoading = targetLength === 1800 ? setIsGeneratingSummaryMedio : setIsGeneratingSummaryPequeno; setLoading(true); try { const summaryPrompt = `Resuma o seguinte texto para ter no máximo ${targetLength} caracteres, mantendo a essência e o tom: "${stripHtml(content.conteudo)}"`; const summary = await geminiAPI.generateContent(summaryPrompt); if (targetLength === 1800) { setConteudoMedio(summary); } else { setConteudoPequeno(summary); } } catch (error) { toast.error(`Ocorreu um erro ao gerar o resumo: ${error.message}`); } finally { setLoading(false); } };
