        ...
        <Box component="main" sx={{ flexGrow: 1, p: { xs: 1, sm: 2, md: 3 }, transition: theme.transitions.create('margin', { easing: theme.transitions.easing.sharp, duration: theme.transitions.duration.leavingScreen }) }} >
          ...
          {currentView === 'campaigns' && (
            <>
              ...
              {activeStep === 6 && <VideoGenerator2 onVideoGenerated={(assets) => { setCampaignState(p => ({ ...p, generatedVideos: [...(p.generatedVideos || []), ...assets] })); addPendingAssetMap(Object.fromEntries(assets.flatMap(a => [[a.url, a.blob], [a.thumbnailUrl, a.thumbnailBlob]]).filter(e => e[0]))); }} onUpdateVideos={(videos) => setCampaignState(p => ({ ...p, generatedVideos: videos }))} onNewAsset={addPendingAsset} />}
              {activeStep === 7 && (
                <Publisher
                  settings={settings}
